...
(function () {
    ...
    function calculateHardMath(values, currentPriceStr) {
        ...
        if (values.BBANDS && values.BBANDS.includes('/')) {
            ...
            let upper = parseVal(parts[0]), mid = parseVal(parts[1]), lower = parseVal(parts[2]);
        ...
        if (!isNaN(supertrend)) { if (p > supertrend) bull++; else if (p < supertrend) bear++; else neutral++; }
        ...
        if (!isNaN(adx)) { if (adx > 25) bull++; else neutral++; } else neutral++;
        ...
        if (values.EMA && values.EMA.includes('/')) {
            ...
            let ema20 = parseVal(parts[0]), ema50 = parseVal(parts[1]), ema200 = parseVal(parts[2]);
    ...
    async function buildAutoPrompt() {
      ...
      const rsiText = readIndicatorText("rsi");
      const emaText = readIndicatorText("ema");
      const macdText = readIndicatorText("macd");
      const bbandsText = readIndicatorText("bbands");
      const atrText = readIndicatorText("atr");
      const stochText = readIndicatorText("stoch");
      const adxText = readIndicatorText("adx");
      const ichimokuText = readIndicatorText("ichimoku");
      const obvText = readIndicatorText("obv");
      const keltnerText = readIndicatorText("keltner");
      const sarText = readIndicatorText("sar");
      const vwapText = readIndicatorText("vwap");
      const mfiText = readIndicatorText("mfi");
      const supertrendText = readIndicatorText("supertrend");
      const cciText = readIndicatorText("cci");
    ...
    window.getSignal = async function () {
      ...
      try {
        ...
        const s = {
          ...
          why_html: Array.isArray(j.why) && j.why.length > 0 ? j.why.map(x => `• ${escapeHtml(x)}`).join("<br/>") : "No specific reasoning provided.",
          cancel_html: (j.direction !== "HOLD" && Array.isArray(j.cancel_conditions) && j.cancel_conditions.length > 0) ? j.cancel_conditions.map(x => `• ${escapeHtml(x)}`).join("<br/>") : ""
        };
